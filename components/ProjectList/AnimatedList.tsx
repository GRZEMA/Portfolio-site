		...
		<ul className='self-start space-y-2 md:space-y-4 lg:space-y-6 max-w-prose 2xl:max-w-[85ch] sm:text-2xl lg:text-3xl xl:text-4xl'>
			{items.map((item) => (
				<motion.li
					...
					viewport={{ once: true }}
					className={`relative before:absolute before:w-2 before:h-2 md:before:h-3 md:before:w-3 before:rotate-45 before:bg-text before:left-0 pl-4 md:pl-8 before:top-1/2 before:-translate-y-1/2 transition-colors duration-300 ${
						(item.link || item.email) && 'hover:text-customYellow'
					}`}
